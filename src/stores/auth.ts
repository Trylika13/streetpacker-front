...
export const useAuth = defineStore("auth", () => {
  ...
  // 👑 FONCTION PRIVÉE : Décode le payload du JWT
  const decodeToken = (jwtToken: string): User => {
    try {
      ...
      // Mappage des claims standards .NET vers ton interface User TypeScript
  ...
  const authenticate = (userToken: string) => {
    ...
    // 👑 Le store s'occupe de décoder lui-même le token reçu !
